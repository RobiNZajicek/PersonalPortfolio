...
import rev from '@/public/assets/rev.jpeg';
...
const ReviewsCarousel = () => {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      if (width >= 1600) {
        ...
        setMaxWidth('800px'); // 1200px - 1600px
  ...
  return (
    ...
    >
      ...
      <div className='absolute inset-0' style={{ backgroundImage: `url(${rev.src})`,alt:"recenze-img", backgroundSize: 'cover', backgroundPosition: 'center' }}></div>
      <motion.h2
              ...
              >
                <div className='flex text-2xl'>
                  {Array(review.stars).fill().map((_, idx) => (
                    <span key={idx} className='text-primary text-[30px] sm:text-[30px] md:text-[30px] lg:text-[30px] Dosxl:text-[35px] Dosxl:text-4xl'>★</span>
                  ))}
                </div>
                <p className='mt-2 sm:mt-2 md:mt-4 xl:mt-6 text-[14px] sm:text-[16px] md:text-[15px] lg:text-[18px] Dosxl:text-[20px] text-white z-20 font-sans h-40 sm:h-36 md:h-24 lg:h-32 xl:h-36'>{review.review}</p>
                <p className='mt-6 font-bold text-primary text-xl z-20 font-sans text-[14px] sm:text-[16px] md:text-[15px] lg:text-[18px] Dosxl:text-[20px]'>{review.name}</p>
